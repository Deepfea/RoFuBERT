...
def select_mutants(dataset_name, model_name, load_model_path, load_step_path, load_step1_path):
    load_test_path = os.path.join(load_step_path, model_name, dataset_name)
    test_output = np.load(os.path.join(load_test_path, 'step0_test_outputs.npy'))
    load_mutant_path = os.path.join(load_step1_path, model_name, dataset_name)
    mutants = pd.read_csv(os.path.join(load_mutant_path, 'step1_seed_mutants.csv'))
    # print(mutants)
    ...
    for output_num in range(len(mutants_output)):
        ...
        ori_index = mutants.loc[output_num, 'belong']
        temp_test_output = test_output[ori_index]
        temp_test_label = np.argmax(temp_test_output)
        if temp_mutant_label == temp_test_label:
    ...
    final_mutants.to_csv(os.path.join(load_mutant_path, 'step1_final_mutants.csv'), index=False)
    np.save(os.path.join(load_mutant_path, 'step1_final_mutants_output.npy'), final_mutants_output)
