...
def select_segs(dataset_name, model_name, load_step_path, load_step1_path, important_num):
    load_seg_path = os.path.join(load_step_path, model_name, dataset_name)
    seg_arr = np.load(os.path.join(load_seg_path, 'step0_test_final_segs.npy'), allow_pickle=True)
    value_arr = np.load(os.path.join(load_seg_path, 'step1_test_seg_importance_values.npy'), allow_pickle=True)
    imp_seg_list, imp_sco_list = select_seg(seg_arr, value_arr, important_num)
    save_seg_path = os.path.join(load_step1_path, model_name, dataset_name)
    if not os.path.exists(save_seg_path):
        os.makedirs(save_seg_path)
    np.save(os.path.join(save_seg_path, 'step1_select_segs.npy'), np.array(imp_seg_list))
    np.save(os.path.join(save_seg_path, 'step1_select_segs_values.npy'), np.array(imp_sco_list))
...
def cal_word_scores(labels, dataset_name, model_name, load_model_path ,load_step_path):
    ...
    test_data = pd.read_csv(os.path.join(load_seed_path, 'step0_test.csv'))
    seg_list = np.load(os.path.join(load_seed_path, 'step0_test_segs.npy'), allow_pickle=True)
    ori_output_list = np.load(os.path.join(load_seed_path, 'step0_test_outputs.npy'), allow_pickle=True)
    ...
    np.save(os.path.join(load_seed_path, 'step0_test_final_segs.npy'), np.array(final_segs))
    print(len(data_df))
    ...
    np.save(os.path.join(load_seed_path, 'step1_test_seg_importance_values.npy'), value_arr)
