...
def test(model_name, dataset_name, load_path):
    # print(model_name)
    # print(dataset_name)
    ...
    test_data_path = os.path.join(load_path, model_name, dataset_name, 'test.csv')
    test_data = pd.read_csv(test_data_path)
    test_dataset = MyDataset(test_data, tokenizer, dataset_name)
    print('测试样本数量：' + str(len(test_data)))
    ...
    model.eval()
    adv_loader = DataLoader(test_dataset, batch_size=128)
    total_acc_test = 0
    with torch.no_grad():
        for test_input, test_label in tqdm(adv_loader):
            ...
            total_acc_test += acc
    result = total_acc_test / float(len(test_data))
