...
def get_adv_data(model_name, dataset_name, load_path, save_path):
    ...
    load_data_path = os.path.join(load_path, 'Argot', str(important_num), model_name, dataset_name, 'step1_final_mutants.csv')
    mutant = pd.read_csv(load_data_path)
    text_list = mutant['mutant']
    label_list = mutant['label']
    str_list = mutant['word']
    merge_dt_dict = {'text': text_list, 'label': label_list, 'word': str_list}
    adv_data2 = pd.DataFrame(merge_dt_dict)

    adv_data = pd.concat([adv_data1, adv_data2], ignore_index=True)
    save_data_path = os.path.join(save_path, model_name + '_' + dataset_name + '_adv.csv')
