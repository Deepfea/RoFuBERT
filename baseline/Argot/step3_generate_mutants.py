...
from baseline.Argot.get_word.get_glyph_word import get_glyph_word
from baseline.Argot.get_word.get_pinyin_word import get_pinyin_word
from baseline.Argot.get_word.get_shuffle_word import get_shuffle_word
from baseline.Argot.get_word.get_splitting_word import get_splitting_word
from method.step1_fuzz_testing.get_sentence.get_sentence import get_sentence


def get_sentences(fact, temp_segs, shuffle_seg_list, pinyin_seg_list, splitting_seg_list, glyph_seg_list):
    sentences = []
    ori_strs = []
    rep_strs = []
    type_list = []
...
def generate_mutant1(dataset_name, model_name, load_step_path, load_step1_path):
    load_seed_path = os.path.join(load_step_path, model_name, dataset_name)
    load_seg_path = os.path.join(load_step1_path, model_name, dataset_name)
    seg_list = np.load(os.path.join(load_seg_path, 'step1_select_segs.npy'), allow_pickle=True)
    seeds = pd.read_csv(os.path.join(load_seed_path, 'step0_test.csv'))
    start_flag = 0
    for seed_num in tqdm(range(len(seeds))):
        ...
        sentences, ori_strs, rep_strs, type_list = get_sentences(fact, temp_segs, shuffle_seg_list, pinyin_seg_list, splitting_seg_list, glyph_seg_list)
        # print(len(sentences))
        # print(len(ori_strs))
        # print(len(rep_strs))
        belong_list = []
    ...
    save_path = os.path.join(load_seg_path, 'step1_seed_mutants.csv')
